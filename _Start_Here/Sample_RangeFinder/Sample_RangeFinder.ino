/*
 * Wired Wizards 2017 LED example code with RangeFider!
 * by Benjamin Davis
 * 
...
class RangeFinder  {
  private:
    int pin; // the input pin on the board
    double conversionRate; // the rate from the value of the output to inches. (units per inch)
  public:
    RangeFinder(int pin, int conversionRate);
    int getVal();
    double getInches();
};
...
RangeFinder goo = RangeFinder (A0, 10); //sets up a RangeFinder on analog pin 0 with a conversion rate of 10 units per inch. (10 is an arbitrary number, to be adjusted later.
...
void loop() {
  ...
  if (Serial.available() > 0) { //checks if there is anything waiting on the serial port
    ...
    switch (incoming) { 
      ...
      case '?': // responds to the Rio with the RangeFinder's reading in inches.
        Serial.println(goo.getInches());
...
RangeFinder::RangeFinder(int pin, int conversionRate){
  this->pin = pin;
  this->conversionRate = conversionRate;
}

int RangeFinder::getVal() {
  return analogRead(this->pin);
}

double RangeFinder::getInches(){
  int val = this->getVal();
  double inches = val / this->conversionRate;
}
